...
function App() {
  ...
  const formSubmit = async (e) => {
    ...
    try {
      ...
      await createEvent({ name, amount, description, image, timestamp: new Date() })
      getEvents();
    } catch (error) {
      console.log(error);
    }
          ...
          {events.map(event => <div className="col">
            <div className="card mb-4 rounded-3 shadow-sm">
              ...
              <div className="card-body">
                <h1 className="card-title pricing-card-title">${event.amount/1000000000000000000000000}<small className="text-muted fw-light">NEAR</small></h1>
                <img width={200} src={event.image} alt="" />
                <p className="list-unstyled mt-3 mb-4">
                  {event.description}
                </p>
                {!event.booked ? <button type="button" onClick={() => book(event.id, event.amount)} className="w-100 btn btn-lg btn-primary">Book Event</button>
                  : event.owner === account.accountId ?
                    <button type="button" onClick={() => sell(event.id)} className="w-100 btn btn-lg btn-outline-danger">Sell Slot</button>
